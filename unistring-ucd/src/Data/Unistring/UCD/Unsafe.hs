...
{-|
Module      : Data.Unistring.UCD.Unsafe
...
-- via "Data.Unistring.UCD.Unsafe"); use 'Enum' instance to access integer
-- value.
--
-- @since 0.1.0.0
newtype CodePoint =
  ...
  -- assume that.  In particular, trying to call "Data.Unistring.UCD" functions
  ...
  succ x
    | x == maxBound =
      errorWithoutStackTrace
        "Enum.succ{Data.Unistring.UCD.CodePoint}: tried to take `succ' of maxBound"
    | otherwise = CodePoint (getCodePoint x + 1)
  pred x
    | x == minBound =
      errorWithoutStackTrace
        "Enum.pred{Data.Unistring.UCD.CodePoint}: tried to take `pred' of minBound"
    | otherwise = CodePoint (getCodePoint x - 1)
  toEnum x
    | x < 0 || x > 0x10FFFF =
      errorWithoutStackTrace
        "Enum.toEnum{Data.Unistring.UCD.CodePoint}: value out of bounds"
